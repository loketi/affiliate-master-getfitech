...
(function() {
  ...
  Jwplayer.prototype = {
    ...
    renderPlayer: function(divId, settings, playerContainerEl, playerIndex) {
      ...
      } else if (window.bordeaux && window.bordeaux.cmd) {
        window.bordeaux.cmd.push(function() {
          var videoSettings = window.jwplayer.defaults;
          videoSettings.playlist = settings.playlist;
          videoSettings.autostart = settings.autostart;

          if (videoSettings &&
            videoSettings.advertising &&
            videoSettings.advertising.schedule &&
            videoSettings.advertising.schedule.length > 0 &&
            videoSettings.advertising.schedule[0].tag
          ) {
            var search = new URLSearchParams(videoSettings.advertising.schedule[0].tag);
            if (search.has('iu')) {
              search.set('iu', self.adunitAccount);
              videoSettings.advertising.schedule[0].tag = decodeURIComponent(search.toString());
            }
          }

          window.bordeaux.video.setup(playerInstance, videoSettings);
        });
      }
